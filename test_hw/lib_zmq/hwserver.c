...
//    }
    ...
    //  set config
    sprintf(configFile,"%s","/Users/PLaTanate/Desktop/transportlayer/TransportLayer/install/config/config1.conf");
    ret = TPTransportInitFromConfigFile(&context, configFile);
    if (ret == 0) {
        getIdFromConfig(context, configFile, deviceID);
        
        char path[256];
        system("mkdir -p /Users/PLaTanate/Desktop/transportlayer/TransportLayer/install/chat_history");
        sprintf(path, "/Users/PLaTanate/Desktop/transportlayer/TransportLayer/install/chat_history/%s.db", deviceID);
    }
    else {
